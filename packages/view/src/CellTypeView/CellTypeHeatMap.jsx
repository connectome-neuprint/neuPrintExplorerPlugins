...
function hexToRGB(hexColor) {
  ...
  const rgbArray = strippedHex.match(/.{1,2}/g).map(function (hex) {
...
function rgbToHex(rgbArray) {
  ...
  const hexStrings = rgbArray.map(function (rgbValue) {
    return rgbValue.toString(16).padStart(2, '0');
  });
...
export default function CellTypeHeatMap({ data, median, neuronInfo }) {
  ...
  useEffect(() => {
    ...
    context.stroke();
  }, [data, colSize, rowSize, maxValue, median]);
