...
test('TablePaginationActions shows prev link disabled', () => {
  const raw = <TablePaginationActions count={50} onChangePage={doNothing} rowsPerPage={5} page={0} />;
  const component = renderer.create(raw);
  const tree = component.toJSON();
  expect(tree).toMatchSnapshot();
...
test('clicking on last page link sends user to last page', () => {
  ...
  const raw = <TablePaginationActions count={50} onChangePage={onChangePage} rowsPerPage={5} page={0} />;
  const wrapper = mount(raw);
  wrapper.find('button').at(3).simulate('click');
  expect(onChangePage.calledOnce).toBe(true);
  expect(onChangePage.args[0][1]).toEqual(9);
});

test('clicking on first page link sends user to last page', () => {
  const onChangePage = sinon.spy();
  const raw = <TablePaginationActions count={50} onChangePage={onChangePage} rowsPerPage={5} page={5} />;
  const wrapper = mount(raw);
  wrapper.find('button').at(0).simulate('click');
  expect(onChangePage.calledOnce).toBe(true);
  expect(onChangePage.args[0][1]).toEqual(0);
});

test('clicking on next page link sends user to next page', () => {
  const page = 4
  const onChangePage = sinon.spy();
  const raw = <TablePaginationActions count={50} onChangePage={onChangePage} rowsPerPage={5} page={page} />;
  const wrapper = mount(raw);
  wrapper.find('button').at(2).simulate('click');
  expect(onChangePage.calledOnce).toBe(true);
  expect(onChangePage.args[0][1]).toEqual(page + 1);
});
